...
**前置需求:** Node 20+,以及本機跑著 [Ollama](https://ollama.com/) 做 embedding(全本地,不需要任何雲端帳號):
...
**現況:** 每天在真實環境 dogfood —— Memory River 是作者自己 agent 群的現役記憶層,同時以 OpenClaw plugin 與 Claude Code push 層兩種形態跑在日常真對話上。每個改動都過 CI(Node 20/22):乾淨檢出 build、全 workspace typecheck、完整測試。1.0 前 API 仍可能變動。
