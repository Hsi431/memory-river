...
*cat5 是對抗類別 —— 「標準答案」是誘答,正解為拒答/糾正,故比照 mem0 的協定不併入 headline,獨立列為穩健性軸(本次 run 在嚴格 judge 下是 22.0%;先前報的「拒答準確率」口徑尚未用現行 judge 重測)。*

**跟上一版公開數字的差別。** 上表**不能**跟我們先前公布的 60.6% 相減,而且差距也不全是進步。有兩件事同時改變:

1. **judge 換了。** 先前的數字由 `gemini-2.5-flash` 評分,Google 已停用該模型、我們兩把 key 都回 `API_KEY_INVALID`,因此改用 xAI `grok-4-1-fast-non-reasoning`。光是換 judge 就會移動絕對分數。
2. **檢索修好了**(混合檢索算了 fused score 卻從沒拿它排序;embedding 的 query 前綴不是模型官方的 instruction 模板)。

為了把這兩件事拆開,我們用**同一個 judge、同一份入庫快照**,把全量 1,986 題**跑了兩趟** —— 一趟修正後、一趟修正前的那個 commit,只有引擎不同:

| | 修正前 | 修正後 | 差 |
|:---|---:|---:|---:|
| 整體(cat 1–4) | 64.5% | **67.3%** | **+2.9pp** |
| 全量 1,986 題 | 54.9% | 57.2% | +2.2pp |

cat 1–4 的配對 McNemar:95 題變錯、139 題變對,χ² = 7.90(p < 0.01)。

也就是說,舊的 60.6% 到今天的 67.3% 之間,**大約 +2.9pp 是引擎真的變好,其餘來自換 judge**。我們這樣報,是因為 benchmark 數字的價值不會超過它旁邊那份協定。
...
- 評測期間關閉記憶寫入(`MR_OTTER_READONLY=1`),讓中斷的 run 可以續跑而不會讓 agent 在評測途中改動記憶庫;上面兩趟對照都跑在同一個設定下

這刻意比常見的排行榜式設定更嚴。它量的是 Memory River 作為「嵌進真實 agent 的通用記憶引擎」,不是一條為 benchmark 特化的答題流水線。

為了對照,我們也曾在**完全相同的那批生成答案**上,另外報一個 mem0 式的 LLM-as-judge 分數。這不動 agent、不動檢索到的 context、不動模型輸出 —— 只換 judge rubric,而光是 rubric 就能把分數拉動很大:在先前那次 run,同一批 cat1 答案在嚴格的不給部分分 judge 下是 **34.8%**,在 mem0 式 rubric 下變 **84.8%**。這就是為什麼 LoCoMo 數字不附完整評測協定就不該被拿來比。

在 mem0 式 rubric 下(rubric prompt 從 `mem0ai/memory-benchmarks` 逐字複製),Memory River 的 cat1–4 在**先前那次以 Gemini 評分的 run** 達 **79.9%**,judge parse failure 一律算錯(17/1,539)。**換 judge 與檢索修正之後,那一輪 rubric 評分尚未重跑,所以我們不把它當成現行數字重述。** 我們把它當「跟公開數字可對照」的比較值,不是主要的工程分數 —— 主數字仍是那個較嚴的端到端分數。
